...
async function main() {
  ...
  try {
    ...
    const existing = await db.listCollections({ name: "final_grades" }).toArray();
    if (existing.length) {
      await db.collection("final_grades").drop();
      console.log("✓ Dropped existing collection 'final_grades'");
    }
    ...
    await db.createCollection("final_grades", { validator });
    console.log("✓ Created 'final_grades' collection with schema validation");
    ...
    // 5. Show collections in 'final_grades'
    const cols = await db.listCollections().toArray();
    console.log("\nCollections in 'final_grades':");
    console.dir(cols, { depth: null });

    // 6. Show the validator
    const info = await db.listCollections({ name: "final_grades" }, { nameOnly: false }).toArray();
    console.log("\nValidator for 'final_grades':");
